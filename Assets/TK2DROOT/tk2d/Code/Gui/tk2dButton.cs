...
public class tk2dButton : MonoBehaviour 
{
	...
	void Update ()
	{
		...
		Ray Mray = viewCamera.ScreenPointToRay(Input.mousePosition);
		RaycastHit MhitInfo;
		if (GetComponent<Collider>().Raycast(Mray, out MhitInfo, 1.0e8f))
		{
			if (!Physics.Raycast(Mray, MhitInfo.distance - 0.01f))
			{
				sprite.color = Global.changeAlpha(sprite.color , 1.0f );
			}else{
				sprite.color = Global.changeAlpha(sprite.color , 0.5f );
			}
		}else{
			sprite.color = Global.changeAlpha(sprite.color , 0.5f );
